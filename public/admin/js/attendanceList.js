...
jsonRequestFBKey.onload = () => {
	...
	jsonRequestFBKey.response.forEach((element) => {
		for (let user_row of user_rows) {
			if (
				user_row.getElementsByClassName("user-id").item(0).innerHTML ==
				element.user_id
			) {
				user_row
					.getElementsByClassName("app-installed")
					.item(0).innerHTML = "Y";
				user_row.getElementsByClassName("keys").item(0).checked = true;
				user_row
					.getElementsByClassName("keys")
					.item(0).disabled = false;
				user_row.getElementsByClassName("keys").item(0).value =
					element.user_id;
				user_row.getElementsByClassName("keys").item(0).onclick =
					onCheckClicked;
			}
		}
	});
...
var setJsonReqAttend = () => {
	if (node_college.value == "") {
		...
		var key = Object.keys(college_map).find(
			(key) => college_map[key] === node_college.value
		);
		jsonRequestAttend.open("GET", URL_ATTEND_LIST + key);
		jsonRequestAttend.send();
	} else {
		var key = Object.keys(depart_map).find(
			(key) => depart_map[key] === node_depart.value
		);
		jsonRequestAttend.open("GET", URL_ATTEND_LIST + key);
		jsonRequestAttend.send();
	}
...
document.querySelector("#college").onchange = () => {
	...
	for (let element of departs) element.hidden = true;

	if (college != null) {
		var departs_selected = document.getElementsByClassName(college.id);

		for (let element of departs_selected) element.hidden = false;
	}
};
